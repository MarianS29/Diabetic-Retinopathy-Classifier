...
def main():
    ...
    # Show an image from each dataset used in training or testing
    print(f"Sanity check: o imagine din fiecare dataset pentru experimentul '{args.experiment}':")
    for source in set([train_source] + test_sources):
        try:
            ds = make_dataset(
                source,
                split='train',
            ...
            plt.imshow(img)
            plt.title(f"{source} | Label: {label}")
            plt.axis('off')
            plt.show()
