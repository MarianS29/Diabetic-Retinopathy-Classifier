...
def main():
    ...
    print("Sanity check: o imagine din dataseturile custom folosite:")
    try:
        ds = make_dataset(train_roots, split='train', image_size=args.img_size, train_augment=args.train_augment)
        img, label = ds[0]
        img = denormalize_image(img)
        plt.figure(figsize=(4, 4))
        plt.imshow(img)
        plt.title(f"{train_source}/train | Label: {label}")
        plt.axis('off')
        plt.show()
    except Exception as e:
        print(f"   Nu am putut incarca o imagine din train: {e}")

    for source, root in test_roots_by_source.items():
        try:
            ds = make_dataset(root, split='test', image_size=args.img_size, train_augment=args.train_augment)
            img, label = ds[0]
            img = denormalize_image(img)
            plt.figure(figsize=(4, 4))
            plt.imshow(img)
            plt.title(f"{source}/test | Label: {label}")
            plt.axis('off')
            plt.show()
        except Exception as e:
            print(f"   Nu am putut incarca o imagine din {source}: {e}")
