...
def show_dataset_samples_grid(dataset_dict, title_prefix=""):
    ...
    for row_idx, (source_name, ds) in enumerate(dataset_dict.items()):
        ...
        for col_idx in range(5):
            ...
            else: ax.text(0.5, 0.5, 'Lipsa', ha='center', va='center')
...
def save_gradcam_overlay_all_classes(model, data_loader, loss_name, device, base_name, results_dirs, show=False):
    ...
    for c in range(5):
        ...
        ax_orig.imshow(image); ax_orig.set_title(f'Original (Adevarat: {c})'); ax_orig.axis('off')
        ax_cam.imshow(overlay); ax_cam.set_title(f'Grad-CAM (Predictie: {preds[0].item()})'); ax_cam.axis('off')
        fh.remove(); bh.remove()
