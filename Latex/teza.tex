...
\campus{Leu}
\program{Tehnici Avansate de Imagistică Digitală}
\titlulobtinut{Master}
...
\texttt{my\_dataset.py}, regăsit în anexa \ref{anexa_cod} se ocupă de încărcarea imaginilor și de aplicarea transformărilor.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth]{Figuri/schema_flux_sistem_principal_doua_etape.png}
    \captionsetup{justification=centering}
...
\(F_2\), ceea ce păstrează accentul pe sensibilitatea față de clasa patologică. Pe setul de test,
raportul actualizat indică o acuratețe de aproximativ \(0.771\), precision de \(0.764\) și recall de
\(0.784\) pentru clasa \textit{Bolnav}. Față de rularea anterioară, modelul binar devine mai echilibrat:
reduce numărul de false pozitive, dar menține suficient de multe cazuri patologice trimise către etapa
de gradare.
\bigskip
...
pentru a favoriza sensibilitatea față de cazurile patologice. Astfel, o acuratețe de test de
aproximativ \(0.771\) trebuie interpretată împreună cu recall-ul de \(0.784\) pentru clasa
\textit{Bolnav}: modelul acceptă în continuare o parte dintre falsele pozitive, dar separarea este mai
echilibrată decât în rularea precedentă.
\bigskip
...
comportamentul actualizat al detectorului binar. Modelul identifică \(5217\) imagini patologice din
\(6652\), ceea ce corespunde unui recall de aproximativ \(0.784\) pentru clasa \textit{Bolnav}. În
același timp, apar \(1616\) false pozitive, adică imagini sănătoase trimise mai departe către modelul
de severitate. Comparativ cu varianta anterioară, matricea indică un compromis mai bun între
sensibilitate și specificitate: numărul de imagini sănătoase direcționate greșit către etapa a doua
scade, iar detectorul păstrează o capacitate bună de a reține cazurile patologice. Falsele pozitive nu
pot fi corectate automat de etapa a doua, deoarece modelul de severitate primește doar clase patologice
și va atribui inevitabil o clasă din intervalul 1--4.
\bigskip
...
matricilor de confuzie. Pentru detectorul binar, scorurile celor două clase sunt apropiate, cu valori
F1 în jurul lui \(0.77\). Clasa patologică păstrează un recall ușor mai mare decât precision-ul, ceea ce
este potrivit pentru rolul de prim filtru al pipeline-ului. În ansamblu, modelul binar nu este optimizat
pentru o decizie izolată de diagnostic, ci pentru separarea inițială a imaginilor fără boală de cele care
trebuie analizate mai detaliat de clasificatorul 1--4.
\bigskip
...
\chapter {Cod\label{anexa_cod}}

\lstinputlisting[language=python, caption=codul de antrenare pentru sistemul principal]{Cod/diabetic-retinopathy-classifier-FIXED-3ramuri.py}
\vspace{1cm}
 
\lstinputlisting[language=python, caption=my\_dataset.py -- \^{i}ncărcarea setului de date echilibrat pentru experimente]{Cod/my_dataset.py}
\vspace{1cm}

\lstinputlisting[language=python, caption={builder.py -- construirea modelelor, funcțiilor de pierdere și optimizatorilor}]{Cod/builder.py}
\vspace{1cm}

\lstinputlisting[language=python, caption={utils.py -- funcții de vizualizare și pentru antrenare}]{Cod/utils.py}
\vspace{1cm} 

\lstinputlisting[language=python, caption={train.py -- antrenarea, evaluarea și generarea graficelor}]{Cod/train.py}
\vspace{1cm}
