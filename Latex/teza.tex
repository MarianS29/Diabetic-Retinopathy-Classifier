...
\begin{itemize}
  ...
  \item \textbf{Capitolul 5 -- Metodologia propusă:} Se prezintă fluxul general al sistemului, modelele analizate,
    arhitecturile ResNet50, EfficientNet-B3, InceptionV3 și Inception-ResNet-V2, funcțiile de pierdere utilizate,
...
\caption{Structura anatomica a fundului de ochi}
...
\captionsetup{justification=centering}
\caption{Reprezentare grafică a diferențelor vizuale între o imagine de fund de ochi fără semne patologice și una cu retinopatie diabetică proliferativă}
...
\captionsetup{justification=centering}
\caption{Schema generală a unei rețele neuronale convoluționale}
\label{fig:cnn}
\end{figure}
...
\begin{itemize}
  ...
  \captionsetup{justification=centering}
  \caption{Schema unei arhitecturi ResNet50}
  \label{fig:resnet50}
  \end{figure}
  ...
  \captionsetup{justification=centering}
  \caption{Schema unei arhitecturi Inception-v3}
  ...
  \captionsetup{justification=centering}
  \caption{Schema unei arhitecturi EfficientNet-B3}
  ...
  \captionsetup{justification=centering}
  \caption{Schema unei arhitecturi Inception-ResNet-v2}
  \label{fig:incres-v2}
  \end{figure}
...
\ \ \ \ Experimentele din această lucrare nu folosesc un singur set de imagini, ci trei
surse principale, pregătite în același format de tip \texttt{ImageFolder}. Această alegere a fost
necesară deoarece clasificarea retinopatiei diabetice este sensibilă la diferențele de domeniu:
imagini provenite din camere diferite, cu iluminare diferită sau cu grade diferite de
preprocesare pot produce performanțe semnificativ diferite. Prin urmare, pe lângă setul
\textit{Diabetic\_Balanced\_Data}, au fost introduse și variantele \textit{balanced\_aug} și
\textit{APTOS matched}, pentru a testa mai bine robustețea modelelor.
\bigskip

Prima sursă este \texttt{datasets/Diabetic\_Balanced\_Data}, disponibilă pe Kaggle sub denumirea
\textit{Diabetic Retinopathy Balanced} \cite{24_kaggle_diabetic_balanced}. Aceasta conține imagini
color ale fundului de ochi, organizate pentru clasificare multiclasă în cinci grade de
severitate. Setul este aproape echilibrat numeric și reprezintă baza experimentelor inițiale,
deoarece permite compararea arhitecturilor și a funcțiilor de pierdere într-un cadru
controlat.
\bigskip

\begin{figure}[H]
\centering
\includegraphics[width=0.95\columnwidth]{Figuri/imagini_balanced_data.png}
\captionsetup{justification=centering}
\caption{Exemple de imagini din setul \textit{Diabetic\_Balanced\_Data} pentru fiecare clasă de severitate}
\label{fig:diabetic-balanced-data}
\end{figure}

A doua sursă este \texttt{datasets/Diabetic\_Balanced\_Aug\_Ben\_Graham\_matched}, denumită în
cod \texttt{balanced\_aug}. Aceasta este construită pornind de la setul
\texttt{Diabetic\_Balanced\_Aug}, care conține o colecție mai mare de imagini retiniene, dar
cu o distribuție natural dezechilibrată. Pentru a o face compatibilă cu restul fluxului
experimental, imaginile au fost împărțite stratificat în subseturi de antrenare, validare
și testare, apoi au fost preprocesate folosind metoda Ben Graham. Această sursă este
importantă deoarece oferă un volum mai mare de imagini normale și moderate, dar păstrează
o provocare realistă: clasele severe rămân mult mai puțin reprezentate.
\bigskip

\begin{figure}[H]
\centering
\includegraphics[width=0.95\columnwidth]{Figuri/imagini_balanced_aug_ben_graham.png}
\captionsetup{justification=centering}
\caption{Exemple de imagini din setul \textit{Diabetic\_Balanced\_Aug\_Ben\_Graham\_matched} pentru fiecare clasă de severitate}
\label{fig:diabetic-balanced-aug-ben-graham}
\end{figure}

A treia sursă este \texttt{datasets/aptos/aptos\_ben\_graham\_matched}, derivată din APTOS 2019
Blindness Detection \cite{9_chetoui2020explainable}. Setul APTOS este folosit pentru a
verifica dacă modelele antrenate pe un domeniu pot generaliza pe imagini provenite dintr-o
altă distribuție. În proiect, APTOS a fost reorganizat pe clase, balansat prin augmentări
blânde și preprocesat cu aceeași metodă Ben Graham, astfel încât diferențele observate în
rezultate să fie cauzate mai ales de conținutul vizual și de domeniul imaginilor, nu de
un format diferit al fișierelor.
\bigskip

Toate sursele sunt organizate în directoarele \texttt{train}, \texttt{val} și \texttt{test}. În
interiorul fiecărui subset există cinci directoare numerotate de la \texttt{0} la \texttt{4},
corespunzătoare nivelurilor de severitate:
...
Această structură permite extragerea directă a etichetei din numele directorului clasei.
În implementare, clasele \texttt{BalancedDRDataset} și \texttt{AptosDRDataset} parcurg aceste
directoare, salvează căile imaginilor și atașează automat eticheta numerică. Astfel, aceeași
logică de încărcare poate fi folosită pentru \texttt{balanced}, \texttt{balanced\_aug},
\texttt{aptos} și pentru combinațiile acestora.

\section{Distribuția claselor}

\ \ \ \ Distribuțiile au fost calculate direct din structura locală a directoarelor. Tabelul
\ref{tab:distributie-diabetic-balanced} prezintă distribuția pentru
\texttt{Diabetic\_Balanced\_Data}. Setul este aproape uniform pe cele cinci clase, cu o
diferență minoră pentru clasa 1.
...
Setul \texttt{balanced\_aug} are o distribuție diferită, prezentată în Tabelul
\ref{tab:distributie-balanced-aug}. Deși numărul total de imagini este mai mare, clasele
nu sunt echilibrate. Clasa 0 domină numeric, iar clasele 3 și 4 au considerabil mai
puține exemple. Acest lucru îl face util pentru testarea comportamentului modelelor într-un
scenariu mai apropiat de distribuțiile naturale întâlnite în practică.

\begin{table}[H]
\centering
\caption{Distribuția imaginilor în setul \textit{Diabetic\_Balanced\_Aug\_Ben\_Graham\_matched}}
\label{tab:distributie-balanced-aug}
\begin{tabular}{|c|c|c|c|c|}
\hline
\textbf{Clasa} & \textbf{Train} & \textbf{Validare} & \textbf{Test} & \textbf{Total} \\
\hline
...
4 & 495 & 141 & 72 & 708 \\
\hline
\textbf{Total} & \textbf{24573} & \textbf{7019} & \textbf{3516} & \textbf{35108} \\
\hline
\end{tabular}
\end{table}

Pentru APTOS, distribuția finală a fost controlată prin scriptul de reconstruire a setului.
Fiecare clasă conține 1500 de imagini pentru antrenare, 200 pentru validare și 250
pentru testare, după aplicarea augmentărilor blânde și a preprocesării Ben Graham. Distribuția
este prezentată în Tabelul \ref{tab:distributie-aptos-matched}.

\begin{table}[H]
\centering
\caption{Distribuția imaginilor în setul \textit{APTOS Ben Graham matched}}
\label{tab:distributie-aptos-matched}
\begin{tabular}{|c|c|c|c|c|}
\hline
\textbf{Clasa} & \textbf{Train} & \textbf{Validare} & \textbf{Test} & \textbf{Total} \\
\hline
...
4 & 1500 & 200 & 250 & 1950 \\
\hline
\textbf{Total} & \textbf{7500} & \textbf{1000} & \textbf{1250} & \textbf{9750} \\
\hline
\end{tabular}
\end{table}

Prin comparație, \texttt{Diabetic\_Balanced\_Data} și \texttt{APTOS matched} sunt balansate,
în timp ce \texttt{balanced\_aug} păstrează un dezechilibru puternic. Această diferență este
importantă pentru interpretarea rezultatelor: performanța ridicată pe un set echilibrat nu
garantează automat robustețe pe un set dezechilibrat sau pe un set provenit din alt
domeniu.

\section{Construirea seturilor augmentate și preprocesate}

\ \ \ \ Pentru a obține variante comparabile între ele, seturile suplimentare au fost generate prin
scripturi dedicate. Aceste scripturi separă două etape: augmentarea offline, care produce
fișiere noi pe disc, și preprocesarea imaginilor, care standardizează aspectul vizual înainte
de antrenare.
\bigskip

Setul \texttt{balanced\_aug} este construit prin scriptul dedicat conversiei
\texttt{Diabetic\_Balanced\_Aug}. În implementare, fișierul folosit este:

\begin{center}
\small\path{build_diabetic_balanced_aug_ben_graham.py}
\end{center}

Acesta citește imaginile din
\texttt{Diabetic\_Balanced\_Aug/resized\_train\_cropped} și etichetele din fișierul
\texttt{trainLabels\_cropped.csv}. Imaginile sunt indexate după nume, apoi sunt împărțite
stratificat în proporție de 80\% pentru antrenare, 15\% pentru testare și 5\% pentru
validare, folosind seed-ul 42. Fiecare imagine este apoi preprocesată Ben Graham și
salvată într-o structură \texttt{train}/\texttt{val}/\texttt{test}/\texttt{0--4}.
\bigskip

Setul APTOS este refăcut prin scriptul \texttt{rebuild\_aptos\_dataset.py}. Mai întâi, scriptul
copiază imaginile curate din \texttt{aptos\_reorganized}, ignorând fișierele care conțin deja
secvența \texttt{\_aug} în nume. Apoi, pentru fiecare clasă și subset, generează augmentări
până la atingerea țintelor stabilite: 1500 de imagini pe clasă în \texttt{train}, 200 în
\texttt{val} și 250 în \texttt{test}. După această echilibrare, imaginile sunt preprocesate
Ben Graham și salvate în \texttt{aptos\_ben\_graham\_matched}.
\bigskip

Augmentările offline pentru APTOS sunt intenționat moderate. Scriptul aplică flip orizontal
cu probabilitatea 0.5, flip vertical cu probabilitatea 0.15, rotații între -8 și 8 grade,
scalări între 0.97 și 1.03 și translații de maximum 2.5\% din dimensiunea imaginii.
Zonele introduse prin transformări geometrice sunt completate cu gri neutru, pentru a nu
crea margini negre artificiale. Aceste valori au fost alese pentru a simula variații
realiste de achiziție, fără a modifica structura clinică a imaginii.

\section{Preprocesarea Ben Graham}

\ \ \ \ Preprocesarea Ben Graham are rolul de a reduce variațiile de iluminare, marginile negre
și diferențele de scară între imagini. În proiect, această etapă este implementată în
\texttt{ben\_graham\_preprocess\_dataset.py} și este folosită atât pentru APTOS, cât și
pentru varianta \texttt{balanced\_aug}.
\bigskip

Fluxul aplicat fiecărei imagini este următorul:

\begin{enumerate}
  \item se elimină marginile negre prin detectarea regiunilor cu intensitate peste pragul 7;
  \item retina este scalată astfel încât raza vizibilă să fie apropiată de valoarea 300;
  \item imaginea este centrată, decupată sau completată până la dimensiunea de lucru;
  \item se aplică blur Gaussian și o combinație ponderată pentru accentuarea contrastului local;
  \item se aplică o mască circulară, astfel încât zona retiniană să rămână centrală și fundalul să fie uniformizat;
  \item imaginea este redimensionată la 512$\times$512 pixeli;
  \item rezultatul este convertit într-o reprezentare grayscale RGB cu contrast controlat.
\end{enumerate}

Această preprocesare nu înlocuiește augmentarea, ci are un scop diferit. Augmentarea crește
variația exemplelor, în timp ce Ben Graham reduce variația nedorită produsă de condițiile
de captură. Folosirea aceleiași metode pentru APTOS și \texttt{balanced\_aug} ajută la
alinierea aspectului vizual al imaginilor înainte de antrenarea rețelelor.

\section{Încărcarea datelor în experimente}

\ \ \ \ Încărcarea imaginilor este realizată în fișierul \texttt{my\_dataset.py}. Clasa
\texttt{BalancedDRDataset} citește directoarele \texttt{train}, \texttt{val} sau \texttt{test}, parcurge
subfolderele \texttt{0--4}, deschide fiecare imagine cu biblioteca PIL, o convertește în
format RGB și returnează imaginea împreună cu eticheta numerică. Clasa
\texttt{AptosDRDataset} extinde aceeași logică, deoarece APTOS este deja organizat în format
compatibil.
\bigskip

Funcția \texttt{make\_dataset} permite selectarea sursei printr-un nume simbolic:

\begin{itemize}
  \item \texttt{balanced} -- folosește \texttt{Diabetic\_Balanced\_Data};
  \item \texttt{balanced\_aug} -- folosește \texttt{Diabetic\_Balanced\_Aug\_Ben\_Graham\_matched};
  \item \texttt{aptos} -- folosește \texttt{aptos\_ben\_graham\_matched};
  \item \texttt{combined} sau \texttt{balanced\_aptos} -- concatenează \texttt{balanced} și \texttt{aptos};
  \item \texttt{balanced\_aug\_aptos} -- concatenează \texttt{balanced\_aug} și \texttt{aptos}.
\end{itemize}

Pentru seturile combinate se folosește \texttt{ConcatDataset}, astfel încât modelul să vadă
imagini din ambele surse în aceeași etapă de antrenare. În scriptul \texttt{train.py}, aceste
surse pot fi selectate fie prin parametrul \texttt{--experiment}, fie prin parametrii
\texttt{--train\_source} și \texttt{--test\_source}. Opțiunea \texttt{--test\_sources} permite evaluarea
aceluiași model pe mai multe seturi de test, de exemplu separat pe APTOS și pe
Balanced.

\section{Augmentarea datelor}

\ \ \ \ În această lucrare apar două tipuri de augmentare. Primul tip este augmentarea offline,
aplicată înainte de antrenare și salvată pe disc. Aceasta apare în
\texttt{Diabetic\_Balanced\_Data}, unde o parte dintre fișiere au secvența \texttt{\_aug\_} în
nume, și în setul APTOS reconstruit, unde augmentările blânde sunt generate explicit de
\texttt{rebuild\_aptos\_dataset.py}. Al doilea tip este augmentarea online, aplicată la încărcarea
imaginilor în timpul antrenării.
\bigskip

Augmentarea online este controlată prin parametrul \texttt{train\_augment}. Pentru valoarea implicită
\texttt{basic}, transformările aplicate pe subsetul \texttt{train} sunt:

\begin{itemize}
  \item redimensionare la dimensiunea configurată prin \texttt{image\_size};
  ...
  \item \textbf{ColorJitter} cu variații moderate de luminozitate și contrast;
  \item conversie în tensor și normalizare.
\end{itemize}

Pentru \texttt{val} și \texttt{test}, transformările sunt deterministe: redimensionare, conversie
în tensor și normalizare. Această separare este importantă deoarece validarea și testarea
trebuie să măsoare comportamentul modelului pe imagini stabile, fără variații aleatorii
introduse la fiecare rulare.
\bigskip

În experimentele realizate pe seturi deja augmentate offline, parametrul \texttt{train\_augment}
poate fi setat și la \texttt{none}. Această opțiune este utilă pentru a evita o dublă
augmentare prea agresivă, mai ales atunci când imaginile au fost deja generate prin
rotații, translații sau flip-uri. Astfel, pipeline-ul permite compararea între scenarii cu
augmentare online activă și scenarii în care diversitatea provine doar din imaginile salvate
în dataset.

\section{Împărțirea datelor și controlul evaluării}

\ \ \ \ Împărțirea în subseturi de antrenare, validare și testare este păstrată explicit în
structura directoarelor. Pentru \texttt{Diabetic\_Balanced\_Data}, raportul este aproximativ 70\%
antrenare, 20\% validare și 10\% testare. Pentru \texttt{balanced\_aug}, împărțirea este
generată stratificat cu raportul 80\%/5\%/15\%, iar pentru APTOS sunt folosite ținte fixe
pe clasă.
\bigskip
...
\begin{itemize}
  \item \textbf{train} este folosit pentru actualizarea ponderilor modelului;
  \item \textbf{val} este folosit pentru monitorizarea antrenării, alegerea celui mai bun model și mecanisme precum early stopping;
  \item \textbf{test} este folosit doar la final, pentru evaluarea modelului salvat.
\end{itemize}

În \texttt{DataLoader}, subsetul de antrenare este încărcat cu \texttt{shuffle=True}, iar subseturile
de validare și testare cu \texttt{shuffle=False}. În plus, experimentele cross-dataset, precum
\path{balanced_to_aptos}, \path{balanced_aptos_to_aptos} sau
\path{balanced_aug_aptos_to_balanced_aug}, sunt folosite pentru a separa două întrebări:
cât de bine învață modelul distribuția pe care a fost antrenat și cât de bine se transferă
pe imagini provenite dintr-o altă sursă.
\bigskip

Această organizare este esențială pentru interpretarea rezultatelor. Un model poate obține
scoruri foarte bune pe un set echilibrat, dar să aibă performanțe mai slabe pe APTOS sau
pe \texttt{balanced\_aug}, unde distribuția claselor și aspectul imaginilor sunt diferite. De
aceea, în capitolul experimental rezultatele sunt analizate nu doar prin acuratețe, ci și
prin macro F1, AUC, quadratic weighted kappa și matrice de confuzie.
...
\begin{itemize}
  ...
  Configurațiile principale folosesc rata de învățare 0.0001. Adam este potrivit pentru aceste
  experimente deoarece adaptează rata de actualizare pentru fiecare parametru și oferă o
  convergență stabilă în multe probleme de clasificare.
  ...
  învățare este redusă atunci când metrica monitorizată nu se mai îmbunătățește.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\columnwidth]{Figuri/cam_resnet50_focal_loss_adam_LR_0.0001.png}
\captionsetup{justification=centering}
...
\section{Optimizări recente ale protocolului experimental} % TODO proababil scos sau modificat daca nu da bine
...
% TODO nu stiu daca parametrii astia imbunatatesc, sa vedem (legat de primele 2 TODOs)
...
\ \ \ \ Capitolul de rezultate sintetizează experimentele rulate în notebook-ul
\texttt{experriments.ipynb}, rezultatele inițiale și experimentele extinse salvate în
directoarele de rezultate ale notebook-urilor. Prima etapă a urmărit
compararea arhitecturilor și a funcțiilor de pierdere pe setul \textit{Diabetic\_Balanced\_Data}.
Etapele următoare au analizat generalizarea între domenii, prin antrenare pe Balanced, APTOS
sau combinații ale acestora și prin evaluare separată pe fiecare sursă de test.
\bigskip

Metricile principale urmărite au fost acuratețea, AUC-ul multiclasă, macro F1 și, pentru
experimentele recente, QWK. Acuratețea și AUC-ul au fost extrase din output-urile notebook-ului,
iar macro F1 și valorile pe clase au fost calculate din rapoartele de clasificare salvate
în fișierele \texttt{classification\_report} și \texttt{prf1}.

\begin{table}[H]
\centering
\caption{Rezultatele inițiale pe setul \textit{Diabetic\_Balanced\_Data}}
\small
\resizebox{\textwidth}{!}{%
\begin{tabular}{|p{3.0cm}|p{2.3cm}|p{2.0cm}|p{1.8cm}|p{1.6cm}|p{1.6cm}|p{4.4cm}|}
\hline
\textbf{Model} & \textbf{Funcție pierdere} & \textbf{Optimizator} & \textbf{Val. Acc. max.} & \textbf{Test Acc.} & \textbf{Test AUC} & \textbf{Observație} \\
\hline
...
\label{tab:rezultate-initiale}
\end{table}

EfficientNet-B3 cu Focal Loss a obținut o acuratețe de 85.25\%, AUC de 0.9733 și
macro F1 de 0.8500, ceea ce îl plasează între ResNet50 și arhitecturile Inception.
Varianta EfficientNet-B3 cu Weighted Cross-Entropy a fost mai slabă, cu 80.10\% acuratețe,
AUC 0.9560 și macro F1 0.7989.

\begin{table}[H]
\centering
\caption{Analiza parametrului \texttt{gamma} pentru ResNet50 cu Focal Loss pe Balanced}
\small
\resizebox{\textwidth}{!}{%
\begin{tabular}{|p{2.0cm}|p{2.5cm}|p{2.5cm}|p{2.5cm}|p{5.0cm}|}
\hline
\textbf{Gamma} & \textbf{Test Acc.} & \textbf{Test AUC} & \textbf{Macro F1} & \textbf{Interpretare} \\
\hline
1.5 & 88.61\% & 0.9811 & 0.8853 & Cel mai bun rezultat pe Balanced, dar putin mai slab la matricea de confuzie față de 1.6. \\
\hline
...
\hline
1.8 & 81.09\% & 0.9597 & 0.8102 & Performanță apropiată de \texttt{gamma=1.6}. \\
\hline
...
\label{tab:gamma-focal-loss}
\end{table}

Experimentele cross-dataset au arătat o diferență clară între performanța pe același domeniu
și performanța pe imagini provenite din altă distribuție.

\begin{table}[H]
\centering
\caption{Rezultate cross-dataset și evaluare separată pe domenii}
\small
\resizebox{\textwidth}{!}{%
\begin{tabular}{|p{3.1cm}|p{3.0cm}|p{2.6cm}|p{1.7cm}|p{1.6cm}|p{1.6cm}|p{1.5cm}|}
\hline
\textbf{Model} & \textbf{Train/Val} & \textbf{Test} & \textbf{Acc.} & \textbf{AUC} & \textbf{Macro F1} & \textbf{QWK} \\
\hline
...
Valorile din tabel indică faptul că adăugarea APTOS în antrenare îmbunătățește transferul către
APTOS față de modelul antrenat doar pe Balanced, dar nu elimină complet decalajul de domeniu.
Pentru Balanced Aug, acuratețea este bună, însă macro F1 scade, ceea ce arată că
performanța globală este influențată de distribuția claselor și că unele clase sunt recunoscute
mai slab.

\section{Evoluția funcției de pierdere} 

\ \ \ \ Evoluția funcției de pierdere arată că modelele învață rapid în primele epoci,
după care diferența dintre pierderea de antrenare și cea de validare devine mai
vizibilă. În experimentele inițiale, ResNet50, InceptionV3 și Inception-ResNet-V2 au prezentat
o scădere constantă a pierderii de antrenare, semn că rețelele se adaptează la
imaginile retiniene.
\bigskip

Pentru experimentele cross-dataset, curbele confirmă că antrenarea pe o distribuție mixtă este
mai dificilă decât antrenarea pe Balanced. În cazul Balanced Aug + APTOS, validarea se
stabilizează devreme, iar mecanismul de early stopping se activează după 14 epoci pentru
ResNet50, cu cea mai bună valoare AUC de validare în jurul epocii 4.

\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{../Notebooks/Rezultate/Old/grafice/istoric_incres_v2_focal_loss_adam_LR_0.0001.png}
\captionsetup{justification=centering}
\caption{Evoluția antrenării pentru Inception-ResNet-V2 cu Focal Loss în experimentele inițiale}
\label{fig:istoric-incres-initial}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{../Notebooks/Rezultate/New/grafice/loss/loss_balanced_aug_aptos_to_balanced_aug_aptos_resnet50_focal_loss_adam_LR_0.0001_Gamma_1.6.png}
\captionsetup{justification=centering}
\caption{Evoluția funcției de pierdere pentru ResNet50 pe Balanced Aug + APTOS}
\label{fig:loss-balanced-aug-aptos-resnet}
\end{figure}

\section{Evoluția acurateții pe setul de validare}

\ \ \ \ În experimentele inițiale, acuratețea pe validare crește treptat pentru toate modelele.
ResNet50 ajunge la aproximativ 83--85\% în funcție de funcția de pierdere, InceptionV3
ajunge la 86.1\%, iar Inception-ResNet-V2 atinge 86.7\%. Aceste valori se reflectă
și în testare: Inception-ResNet-V2 obține 86.52\%, iar InceptionV3 86.04\%, depășind
variantele ResNet50.
\bigskip

În experimentele extinse, acuratețea de validare nu mai este suficientă pentru a descrie
generalizarea. De exemplu, modelul ResNet50 antrenat pe Balanced + APTOS ajunge la
62.32\% acuratețe pe APTOS, dar la 90.40\% pe Balanced. Această diferență arată
că validarea combinată poate ascunde diferențe importante între domenii.

\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{../Notebooks/Rezultate/New/grafice/acuratete/acc_balanced_aptos_to_balanced_aptos_resnet50_focal_loss_adam_LR_0.0001_Gamma_1.6.png}
\captionsetup{justification=centering}
\caption{Evoluția acurateții pentru ResNet50 antrenat pe Balanced + APTOS}
\label{fig:acc-balanced-aptos-resnet}
\end{figure}

\section{Analiza scorului AUC}

\ \ \ \ AUC-ul a fost una dintre cele mai stabile metrici în experimentele inițiale. ResNet50
cu Cross-Entropy a obținut AUC 0.9734, ResNet50 cu Weighted Cross-Entropy 0.9730,
InceptionV3 0.9770, iar Inception-ResNet-V2 0.9761. EfficientNet-B3 cu Focal Loss a
obținut AUC 0.9733, confirmând că arhitectura separă bine clasele.
\bigskip

În schimb, AUC-ul scade puternic în transferul între domenii. Balanced $\rightarrow$ APTOS
are AUC 0.7434, iar APTOS $\rightarrow$ Balanced are AUC 0.6352. Prin adăugarea
APTOS în antrenare, AUC-ul pe APTOS crește la aproximativ 0.86--0.88, însă rămâne
sub valorile obținute pe Balanced.

\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{../Notebooks/Rezultate/New/grafice/auc/auc_balanced_aug_aptos_to_balanced_aug_aptos_resnet50_focal_loss_adam_LR_0.0001_Gamma_1.6.png}
\captionsetup{justification=centering}
\caption{Evoluția AUC pentru ResNet50 pe Balanced Aug + APTOS}
\label{fig:auc-balanced-aug-aptos-resnet}
\end{figure}

\section{Analiza curbelor ROC multiclasă}

\ \ \ \ Curbele ROC confirmă tendința observată prin AUC: clasele extreme sunt, în general,
mai ușor de separat, iar clasele intermediare sunt mai dificile. În experimentele pe
Balanced, curbele sunt ridicate pentru majoritatea claselor, ceea ce explică AUC-urile apropiate
de 0.97--0.98.
\bigskip

Compararea curbelor ROC pentru același model evaluat pe APTOS și Balanced arată că
performanța agregată nu este suficientă. Modelul poate păstra o separare foarte bună pe
Balanced, dar poate avea praguri mult mai nesigure pe APTOS.

\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{../Notebooks/Rezultate/Old/grafice/roc_inception_v3_focal_loss_adam_LR_0.0001.png}
\captionsetup{justification=centering}
\caption{Curbe ROC pentru InceptionV3 în experimentele inițiale}
\label{fig:roc-inception-initial}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{../Notebooks/Rezultate/New/grafice/roc/roc_balanced_aptos_to_balanced_aptos_resnet50_focal_loss_adam_LR_0.0001_Gamma_1.6_test_aptos.png}
\captionsetup{justification=centering}
\caption{Curbe ROC pentru ResNet50 antrenat pe Balanced + APTOS și testat pe APTOS}
\label{fig:roc-balanced-aptos-test-aptos}
\end{figure}

\section{Analiza matricelor de confuzie}

\ \ \ \ Matricile de confuzie arată că modelele obțin cele mai bune rezultate pentru clasele
avansate, în special clasele 3 și 4, unde semnele patologice sunt mai vizibile. Pentru
ResNet50 cu \texttt{gamma=1.6} pe Balanced, raportul de clasificare indică F1 de 0.9553
pentru clasa 3 și 0.9807 pentru clasa 4. În schimb, clasele 0, 1 și 2
sunt mai apropiate vizual și produc confuzii mai frecvente.
\bigskip

Această distribuție a erorilor este importantă clinic. O confuzie între clase vecine poate
reflecta tranziția graduală a bolii, însă o confuzie între o imagine fără retinopatie
și o formă proliferativă ar avea risc mult mai mare. În experimentele cross-dataset,
matricile de confuzie pe APTOS arată o degradare mai puternică a claselor intermediare.

\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{../Notebooks/Rezultate/New/grafice/matrici_confuzie/cm_norm_balanced_to_balanced_resnet50_focal_loss_adam_LR_0.0001_Gamma_1.6.png}
\captionsetup{justification=centering}
\caption{Matrice de confuzie normalizată pentru ResNet50 pe Balanced, \texttt{gamma=1.6}}
\label{fig:cm-resnet-balanced-gamma16}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{../Notebooks/Rezultate/New/grafice/matrici_confuzie/cm_norm_balanced_aptos_to_balanced_aptos_resnet50_focal_loss_adam_LR_0.0001_Gamma_1.6_test_aptos.png}
\captionsetup{justification=centering}
\caption{Matrice de confuzie normalizată pentru ResNet50 antrenat pe Balanced + APTOS și testat pe APTOS}
\label{fig:cm-resnet-balanced-aptos-test-aptos}
\end{figure}

\section{Comparație între modelele testate}

\ \ \ \ Compararea arhitecturilor arată că modelele mai complexe aduc un avantaj în experimentele
inițiale pe Balanced. InceptionV3 obține 86.04\% acuratețe și AUC 0.9770, iar
Inception-ResNet-V2 obține 86.52\% acuratețe și AUC 0.9761. ResNet50 rămâne competitiv,
dar variantele sale inițiale se opresc între 82.94\% și 84.73\% acuratețe. EfficientNet-B3
cu Focal Loss ajunge la 85.25\%, confirmând că scalarea eficientă poate oferi un
compromis bun între performanță și complexitate.
\bigskip

În scenariile cross-dataset, diferența dintre arhitecturi este mai puțin importantă decât diferența
dintre domenii. ResNet50 și Inception-ResNet-V2 pot obține scoruri bune pe Balanced, dar
performanța pe APTOS rămâne limitată. Inception-ResNet-V2 pe Balanced Aug + APTOS obține
66.00\% acuratețe și macro F1 0.6572 pe APTOS, ușor peste ResNet50 în același
scenariu, dar pe Balanced\_Aug scorul macro F1 rămâne redus, 0.5414, deși acuratețea este
82.00\%.

\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{../Notebooks/Rezultate/New/grafice/precision_recall_f1/prf1_balanced_aptos_to_balanced_aptos_resnet50_focal_loss_adam_LR_0.0001_Gamma_1.6_test_balanced.png}
\captionsetup{justification=centering}
\caption{Precision, recall și F1 pe clase pentru ResNet50 testat pe Balanced după antrenare Balanced + APTOS}
\label{fig:prf1-balanced-aptos-test-balanced}
\end{figure}

\section{Interpretarea erorilor de clasificare}

\ \ \ \ Erorile de clasificare apar în principal în clasele intermediare, unde diferențele clinice
și vizuale sunt mai subtile. În rapoartele de clasificare, clasele 3 și 4 au
de obicei scoruri F1 ridicate, în timp ce clasele 0--2 au scoruri mai mici
și sunt mai sensibile la domeniul de testare.
\bigskip

Pentru interpretarea vizuală a deciziilor, scriptul generează hărți Grad-CAM. Acestea evidențiază
zonele imaginii care au contribuit cel mai mult la predicție. Grad-CAM este util ca
instrument de inspecție, însă nu înlocuiește analiza cantitativă prin metrici și matrici de
confuzie.

\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{../Notebooks/Rezultate/New/grafice/gradcam/gradcam_balanced_aptos_to_balanced_aptos_resnet50_focal_loss_adam_LR_0.0001_Gamma_1.6_test_balanced.png}
\captionsetup{justification=centering}
\caption{Exemplu Grad-CAM pentru ResNet50 evaluat pe Balanced după antrenare Balanced + APTOS}
\label{fig:gradcam-resnet-balanced-aptos}
\end{figure}

\bigskip

Concluzia generală a capitolului este că modelele pot atinge performanțe ridicate pe
testul provenit din aceeași distribuție, în special pe Balanced, dar generalizarea între seturi
de date rămâne dificilă. Cele mai bune rezultate inițiale sunt obținute de Inception-ResNet-V2
și InceptionV3, iar EfficientNet-B3 se comportă competitiv. Pentru utilizare practică, rezultatele
pe APTOS arată că este necesară o strategie mai puternică de adaptare la domeniu.
...
Predicția este calculată cu \texttt{torch.no\_grad()}, pentru a dezactiva calculul gradientului în
timpul inferenței, iar probabilitățile sunt obținute prin softmax \cite{pytorch_docs}.
...
ceea ce oferă o reprezentare interactivă a fundului de ochi în interiorul aplicației. Pentru
această componentă a fost utilizată biblioteca Three.js, care simplifică randarea graficii 3D
în browser prin WebGL \cite{threejs_docs}.
...
\section{Rezultatele obținute față de obiectivele inițiale} % TODO de modificat dupa ce zic de balanced_aug+aptos+restul experimentelor
...
O altă limitare este prezența imaginilor augmentate direct în dataset. % TODO trebuie sa fac ceva si cu asta
Acestea ajută la echilibrarea claselor, dar pot introduce exemple foarte asemănătoare. Pentru o validare
...
\lstinputlisting[language=python, caption=my\_dataset.py -- \^{i}nc\u0103rcarea setului de date echilibrat]{../Notebooks/my_dataset.py}
\vspace{1cm}

\lstinputlisting[language=python, caption=train.py -- antrenarea, evaluarea \c{s}i generarea graficelor]{../Notebooks/train.py}
\vspace{1cm}
