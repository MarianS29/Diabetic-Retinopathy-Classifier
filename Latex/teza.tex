...
\ \ \ \ Noile rezultate susțin decizia de a trata soluția finală ca un pipeline în două etape:
detecție urmată de clasificarea severității. Analizele pe clase și matricele de confuzie arată că
separarea clasei 0 de clasele patologice are o importanță diferită față de diferențierea dintre
stadiile 1--4. De asemenea, erorile observate în matricele de confuzie indică faptul că modelul
confundă mai ales clasele 1 și 2 cu clasa 0, dar nu confundă în aceeași măsură clasele marginale,
cum ar fi clasa 0 și clasa 4. Această distribuție a erorilor justifică separarea problemei: primul
model trebuie să fie sensibil la prezența oricărei forme de boală, iar al doilea model poate fi
specializat pe diferențele mai fine dintre gradele de retinopatie.
\bigskip

Această idee este implementată și testată în notebook-ul
\path{Notebooks/Main/Local/demo_subset_training_local_cuda.ipynb}. Notebook-ul rulează local, cu
acceleratorul setat explicit pe CUDA, și folosește EyePACS ca sursă de date pentru un test rapid pe
subset. Înainte de antrenare, fișierul \texttt{all\_labels.csv} este mapat la imaginile disponibile
pe disc, sunt eliminate intrările fără fișier valid, iar etichetele sunt transformate în două ținte:
\texttt{is\_diseased}, pentru etapa binară, și \texttt{level\_stage2}, pentru clasificarea claselor
1--4. Astfel, aceeași sursă de date produce două probleme de învățare distincte, dar coerente între
ele.
\bigskip

Preprocesarea este realizată offline printr-o combinație Ben Graham + CLAHE. Fiecare imagine este
redimensionată la \(224 \times 224\), este detectată zona utilă a retinei, fundalul este tratat printr-o
mască și un halo gradual, iar contrastul local este îmbunătățit cu CLAHE. După această etapă,
notebook-ul construiește DataLoader-e separate pentru cele două faze. Pentru antrenare sunt folosite
augmentări dinamice moderate: flip orizontal, rotații mici, translații, scalare și variații reduse de
luminozitate și contrast. Pentru validare se păstrează doar transformarea în tensor și normalizarea
ImageNet, astfel încât evaluarea să fie stabilă.
\bigskip

Etapa 1 este formulată ca detecție binară \textit{No DR} versus \textit{DR}. În configurația din
notebook, aceasta folosește \texttt{EfficientNet-B3} cu un singur neuron de ieșire și
\texttt{BCEWithLogitsLoss}. Pentru dezechilibrul dintre imaginile sănătoase și cele patologice este
calculat \texttt{pos\_weight}, astfel încât exemplele pozitive să nu fie ignorate. Etapa 2 este
formulată ca problemă multiclasă pe clasele 1, 2, 3 și 4. Pentru aceasta este folosit
\texttt{Inception-ResNet-v2}, construit prin \texttt{timm}, cu \texttt{CrossEntropyLoss}, ponderi pe
clase și \textit{label smoothing}. Cele două modele au optimizatoare AdamW separate, rate de învățare
diferite și scheduler \texttt{ReduceLROnPlateau} pe baza scorului QWK.
\bigskip

Motorul de antrenare urmărește pentru fiecare epocă pierderea, acuratețea și QWK pe validare, iar cel
mai bun model este păstrat în funcție de QWK. Această alegere este importantă deoarece QWK ține cont
de ordinea claselor și penalizează mai puternic confuziile între stadii îndepărtate. Notebook-ul
include și un modul de evaluare care produce rapoarte de clasificare și matrice de confuzie pentru
fiecare etapă. În acest fel, se poate vedea separat dacă sistemul eșuează la detecția bolii sau la
gradarea severității.
\bigskip

Fluxul de inferență din notebook reflectă direct arhitectura finală. Funcția \texttt{predict\_retina}
primește o imagine, aplică transformarea de validare și rulează mai întâi modelul binar. Dacă
probabilitatea de boală este sub pragul ales, sistemul returnează clasa 0. Dacă probabilitatea
depășește pragul, imaginea este transmisă către modelul de severitate, care produce probabilități
pentru clasele 1--4, iar clasa finală este obținută prin adăugarea offset-ului necesar la predicția
din etapa a doua. Această structură păstrează ieșirea finală în scala completă 0--4, dar permite
analizarea și optimizarea separată a celor două decizii critice: prezența bolii și severitatea ei.
