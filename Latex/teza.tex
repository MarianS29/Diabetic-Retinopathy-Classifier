...
Pentru experimentele recente au fost adăugați parametri suplimentari, precum
\texttt{monitor\_metric}, \texttt{scheduler}, \texttt{amp}, \texttt{grad\_clip},
\texttt{ema\_decay}, \texttt{tta\_eval}, \texttt{model\_dropout},
\texttt{freeze\_backbone\_epochs}, \texttt{backbone\_lr\_mult} și
\texttt{train\_augment}.
Aceștia permit control mai fin asupra criteriului de selecție,
stabilității antrenării și modului de fine-tuning.
\bigskip

Scheduler-ul poate fi selectat între \texttt{plateau}, \texttt{cosine} și \texttt{none}.
În varianta \texttt{plateau}, rata de învățare este redusă atunci când metrica
monitorizată nu se mai îmbunătățește. În experimentele recente, metrica monitorizată
poate fi aleasă explicit, ceea ce permite salvarea modelului în funcție de
AUC, acuratețe, balanced accuracy, macro F1 sau quadratic weighted kappa.
...
astfel încât fiecare clasă să contribuie egal la scorul final.
\bigskip

Pentru retinopatia diabetică, quadratic weighted kappa este relevantă deoarece clasele au
...
\texttt{qwk} ca metrică de monitorizare. În plus, scriptul include generarea de hărți
...
Pentru ResNet50, aceste optimizări sunt folosite împreună cu Focal Loss și
\texttt{gamma=1.6}. Configurația urmărește îmbunătățirea performanței fără a schimba arhitectura de bază.
Pentru Inception-ResNet-V2, care reprezintă modelul principal vizat, protocolul include și fine-tuning
gradual: înghețarea temporară a backbone-ului, rată de învățare mai mică pentru
backbone, dropout configurabil și imagini de intrare mai mari. Aceste setări sunt
menite să protejeze reprezentările preantrenate și să reducă supraînvățarea.
...
După primele rulări, protocolul experimental a fost extins cu o variantă de
antrenare mai robustă. Aceasta păstrează Focal Loss și valoarea \texttt{gamma=1.6}, dar
adaugă monitorizare prin QWK, early stopping, mixed precision, gradient clipping, EMA,
scheduler configurabil și test-time augmentation opțional. Scopul acestor modificări este creșterea
stabilității și reducerea timpului pierdut în epoci în care modelul nu mai
învață suficient.

\begin{table}[H]
\centering
\caption{Experimente recente cu optimizări de antrenare}
\small
\resizebox{\textwidth}{!}{%
\begin{tabular}{|p{1.5cm}|p{2.7cm}|p{4.8cm}|p{3.5cm}|p{3.2cm}|}
\hline
\textbf{Exp.} & \textbf{Model} & \textbf{Configurație} & \textbf{Metrici urmărite} & \textbf{Status} \\
\hline
7-R & ResNet50 & Focal Loss, \texttt{gamma=1.6}, QWK monitor, AMP, grad clipping, EMA,
early stopping, \texttt{train\_augment} configurabil, scheduler configurabil & Acc., AUC, QWK, macro F1, balanced acc. &
în curs; valori de completat \\
\hline
8-I & Inception-ResNet-V2 & Focal Loss, imagini de intrare mai mari, AdamW,
dropout configurabil, backbone înghețat temporar, LR diferențiat, AMP, EMA,
\texttt{train\_augment} configurabil &
Acc., AUC, QWK, macro F1, balanced acc. & în curs; valori de completat \\
\hline
\end{tabular}
}
\end{table}

Rezultatele generate de script sunt salvate în subdirectoare separate, pentru a
facilita compararea experimentelor. Matricele de confuzie, curbele ROC, graficele de loss,
graficele de acuratețe, rapoartele de clasificare, imaginile Grad-CAM și graficele pentru
QWK, macro F1 și balanced accuracy sunt grupate în directoare dedicate. Această
organizare reduce riscul de amestecare a fișierelor atunci când sunt rulate mai
multe configurații succesiv.
...
În experimentele inițiale, AUC-ul de validare a fost folosit frecvent ca
metrică principală pentru salvarea modelului. Această alegere este justificată deoarece o
predicție medicală nu trebuie analizată doar ca etichetă finală, ci și prin
distribuția probabilităților. În experimentele recente, criteriul de selecție a fost generalizat prin
argumentul \texttt{monitor\_metric}. Astfel, modelul poate fi salvat și după QWK, macro F1,
balanced accuracy sau acuratețe, în funcție de obiectivul experimentului.
...
În versiunea recentă, interfața include și informații tehnice suplimentare despre model,
pentru a susține analiza din perspectiva inginerului. Aceste informații arată cum poate
fi conectat un model antrenat la o interfață utilizabilă, nu doar la
un script de evaluare.

\section{Arhitectura aplicației}

\ \ \ \ Aplicația este organizată în două componente principale: un backend Flask și un
frontend React. Backend-ul este responsabil pentru încărcarea modelului, verificarea și preprocesarea imaginii,
precum și calcularea predicției. Frontend-ul este responsabil pentru interfața de utilizare,
selecția modelului, încărcarea imaginii, vizualizarea 3D și afișarea rezultatului.
\bigskip
...
numele modelului, verifică existența fișierului, aplică transformările necesare și rulează inferența. Dacă
imaginea pare să fie o imagine brută, de exemplu din EyePACS, backend-ul aplică
preprocesarea necesară înainte de inferență. Imaginea este convertită în RGB, redimensionată la
dimensiunea așteptată de model, transformată în tensor și normalizată cu valorile ImageNet.
Predicția este calculată cu \texttt{torch.no\_grad()}, iar probabilitățile sunt obținute prin softmax.
...
în zone distincte pentru rezultat, recomandări și metadate tehnice. Aceste zone au
fost echilibrate vizual pentru a avea dimensiuni comparabile și pentru a rămâne
ușor de citit pe rezoluții diferite.
\bigskip

Interfața a fost adaptată pentru utilizare responsive și include un comutator pentru
modul light/dark. De asemenea, aplicația conține o vizualizare 3D a imaginii originale
încărcate, nu a imaginii preprocesate. Utilizatorul poate roti obiectul cu mouse-ul,
ceea ce oferă o reprezentare interactivă a fundului de ochi în interiorul aplicației.
